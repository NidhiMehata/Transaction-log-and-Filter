...
export class AllComponent implements OnInit {
  ...
  submit(form: NgForm) {
    this.transactionService.getFilteredAmount(1,this.selectedOperator,this.selectedAmount,0).subscribe((res) => {
  ...
  refreshTransactionList() {
    this.transactionService.getFilteredTransaction(1).subscribe((res) => {
      this.transactionService.transactions = res as Transaction[];
      this.count = this.transactionService.transactions.length;
      let i;
      this.sum = 0;
